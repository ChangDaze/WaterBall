...
public class ReadStudents {
    ...
    private static Student dataLineToStudent(String line) {
        ...
        String[] left = twoPart[0].split(" ");
        String name = left[0];
        int experience = parseInt(left[1].substring(0, left[1].length()-1)); //去掉y
        String language = left[2];
        String title = left[3];
        List<Integer> slots = stream(twoPart[1].split("\\s|]"))
                .map(Integer::parseInt).toList();
        boolean[] availableTimeSlots = new boolean[13];
        for ( Integer slot : slots) {
            availableTimeSlots[slot-9] = true; //只有9 ~ 21點
        }
