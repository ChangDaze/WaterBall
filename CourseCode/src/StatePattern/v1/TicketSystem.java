...
public class TicketSystem {
    ...
    public void setState(State state) {
        ...
        //好像沒特別控制從CLIENT 變 SOLD OUT，或是也用throw exception ticket != 0 擋住會穩一點?
//        if ( state == SOLD_OUT) {
//            tickets = 0;
//            spitCoins(total);
//        }
        ...
        if ((state == ENOUGH_COINS || state == IN_STOCK) && tickets <= 0) {
            throw new IllegalStateException("Can not set to the IN_STOCK state if tickets <= 0.");
        }
    }
    ...
    public void fillTickets(int tickets) {
        ...
        this.tickets += tickets;
        if (this.tickets > 0 && state == SOLD_OUT) {
            setState(IN_STOCK);
        }
    }
