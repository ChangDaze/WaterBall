...
/**
 * Items 1–6 of the fsm test checklist (DESIGN.md §3).
 */
class StateMachineTest {
    ...
    /** Records every lifecycle call into a shared log. */
    ...
    private StateMachine machineStartingAt(State initial) {
        return new StateMachine(() -> initial);
    }

    @Test
    void start_enters_initial_state() {
        StateMachine m = machineStartingAt(a);
        m.start();
    ...
    void golden_fire_order_handle_exit_action_enter() {
        StateMachine m = machineStartingAt(b);
    ...
    void first_matching_transition_wins_and_order_flips_result() {
        ...
        Supplier<StateMachine> fresh = () -> machineStartingAt(b);
