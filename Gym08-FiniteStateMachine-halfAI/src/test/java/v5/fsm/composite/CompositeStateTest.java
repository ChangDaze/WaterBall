...
public class CompositeStateTest {
    ...
    class NamedComposite extends CompositeState {
        ...
        public NamedComposite(String name, StateMachine inner) {
            super(inner);
            this.name = name;
        }
    ...
    void reentry_resets_inner_and_supplier_is_reevaluated_each_start(){
        ...
        root.handle(new E1()); // leave composite, inner machine stops
        assertNull(innerMachine.currentState(), "inner machine must be stopped when composite exits");

        root.handle(new E2()); // re-enter composite, inner machine restarts, supplier re-resolved
    ...
    void spec_example_a_b_e1_e2_e3_ordering() {
        ...
        log.clear();
        root.handle(new E1()); // nobody cares about E1 -> only inner state reacts
        assertEquals(List.of("handleB1"), log);

        log.clear();
        root.handle(new E2()); // inner transition : handle -> exit -> action -> enter
